...
public class ImageMatrixGUI implements Serializable {
	...
	public void updateUpperBar(Hero hero) {

		this.clearStatus();

		for (int i = 0; i <= 9; i++) {
			this.addStatusImage(new Black(new Position(i, 0)));
		}

		for (int i = 0; i < hero.getFireBalls().size(); i++) {
			Fire fireballs = hero.getFireBalls().get(i);
			fireballs.setPosition(new Position(i, 0));
			this.addStatusImage((ImageTile) fireballs);
		}

		for (int i = 0; i < hero.getInventory().size(); i++) {
			Items items = hero.getInventory().get(i);
			items.setPosition(new Position(i + 7, 0));
			this.addStatusImage((ImageTile) items);
		}

		double hpPercentage = ((double)hero.getHp()/(double)hero.getMaxHp()) * 100;

		int hpPercentageCalculated = (int)hpPercentage;

		if(hpPercentageCalculated == 100)
		{
			this.addStatusImage(new Green(new Position(3,0)));
			this.addStatusImage(new Green(new Position(4,0)));
			this.addStatusImage(new Green(new Position(5,0)));
			this.addStatusImage(new Green(new Position(6,0)));
		}
		else if(hpPercentageCalculated < 100 && hpPercentageCalculated >= 75){
			this.addStatusImage(new Green(new Position(3,0)));
			this.addStatusImage(new Green(new Position(4,0)));
			this.addStatusImage(new Green(new Position(5,0)));
			this.addStatusImage(new GreenRed(new Position(6,0)));
		}
		else if(hpPercentageCalculated < 75 && hpPercentageCalculated >= 50){
			this.addStatusImage(new Green(new Position(3,0)));
			this.addStatusImage(new Green(new Position(4,0)));
			this.addStatusImage(new GreenRed(new Position(5,0)));
			this.addStatusImage(new Red(new Position(6,0)));
		}
		else if(hpPercentageCalculated < 50 && hpPercentageCalculated >= 25) {
			this.addStatusImage(new Green(new Position(3,0)));
			this.addStatusImage(new GreenRed(new Position(4,0)));
			this.addStatusImage(new Red(new Position(5,0)));
			this.addStatusImage(new Red(new Position(6,0)));
		}
		else if(hpPercentageCalculated < 25 && hpPercentageCalculated > 0) {
			this.addStatusImage(new GreenRed(new Position(3,0)));
			this.addStatusImage(new Red(new Position(4,0)));
			this.addStatusImage(new Red(new Position(5,0)));
			this.addStatusImage(new Red(new Position(6,0)));
		}
		else {
			this.addStatusImage(new Red(new Position(3,0)));
			this.addStatusImage(new Red(new Position(4,0)));
			this.addStatusImage(new Red(new Position(5,0)));
			this.addStatusImage(new Red(new Position(6,0)));
		}
